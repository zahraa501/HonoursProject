...
<head>
	...
	else
	{
		...
		echo'<script> function loadpage(){$("body").show(); loadGraphs();
					}</script>';
	}
	mysqli_close($con);	
	?>
...
<body onload="loadpage()" style="display:none">
			...
			<div class="panel-group" id="testInformation">
				...
				<br>
					<?php 
						...
						//course dropdown
						echo '<div class="col-lg-4"><table width="100%"><tr><td rowspan="1"><label>Courses:</label></td><td rowspan="3"><select class="form-control" id="courseSelect" onChange="testDropdown(this.value)">';
						...
						echo '<div class="col-lg-6"><table width="100%"><tr><td rowspan="1"><label>Test:</label></td><td rowspan="2"><select  id="testSelect" class="form-control" onChange="changeDashboard(this.value)">';
							$tests = mysqli_query($con,"SELECT Name FROM test WHERE Course='CSC1010H'");						
							while($row = mysqli_fetch_array($tests))
							{
								echo'<option value="'.$row['Name'].'">'.$row['Name'].'</option>';
							}
						echo '</select></td><td rowspan="2"><label>&nbsp;&nbsp;&nbsp;&nbsp;Aggregate for All Tests:</label>&nbsp;&nbsp;<input type="checkbox" onChange="changeDashboard()" id="all"></td></tr></table></div>';
						...
						$getQuestionTypes = "";
						$testID = "";
						$test = mysqli_query($con,"SELECT testID, TypeOfQuestions FROM test WHERE Course='CSC4000W' AND Name='Test 1'");						
						...
						$scripts = mysqli_query($con,"SELECT * FROM script WHERE Course='CSC4000W' AND Test=".$testID."");						
						...
						//echo "<br>".$first." ".$usecond." ".$second." ".$third." ".$fail." ".$absent;
						$tempMarks = array();
						$amountOfQuestions = 0;
						...
						$count = 0;
						$getStudents = mysqli_query($con,"SELECT DISTINCT StudentNumber FROM registered_students WHERE Course='CSC4000W'");						
				...
				<?php
					...
					var lineChartData = {
							labels : [ <?php for($i = 1; $i <= 3; $i++)
									{
										if($i != sizeOf($questionMark))
										{
											echo '"Student '.$i.'",';
										}
										else
							...
							datasets : [
								{
									...
									data : [
									<?php for($i = 0; $i <$amountOfStudents; $i++)
									{
										if($i < $amountOfStudents-1)
										{
											echo (($studentMarks[$i]/$totalMark)*100).',';
										}
										else
										{
											echo (($studentMarks[$i]/$totalMark)*100);
				...
				<!--<div class="col-sm-6">
					<div>
						<h3 class="text-center">Comparison of Total Marks</h3>
						<canvas id="canvasLine" height="450" width="500"></canvas>
					</div>
				</div>-->
	...
	{
		alert(document.getElementById("courseSelect").value+" , "+document.getElementById("testSelect").value+" and all: "+document.getElementById("all").checked);
	}
	function Logout() {
