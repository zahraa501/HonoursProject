                ...
                <div class="col-lg-12">
					...
					<div class="panel-body">
						...
						echo '<li>
							...
							if($Type == "Admin")
							{
							echo'<li class="active"><a>Add a Course</a></li>';
							}
							if($Type != "Tutor")
							echo '<li>
								<a href="AddTest.php?q='.$user.'">Add a Test</a>
						...
						<form id="upload" action="CourseInformation.php<?php echo '?q='.$user;?>" method="POST">
							<table>
							<tr><th><Label>Course Name: &nbsp;</Label></th>
							<th><input name="Course" class="form-control"><br></th></tr>
							...
							<?php 
								...
								echo '<tr><th><Label>Course Convenor:</Label></th><th><div class=form-group><select name=CC class=form-control>';
								...
								$result = mysqli_query($con,"SELECT Name, StaffID FROM staff");						
								while($courses = mysqli_fetch_array($result))
								{
									$StaffName = $courses['StaffID'];
									echo '<option value='.$StaffName.'>'.$courses['Name'].'</option>';
								}
								
								echo '</select></div></th></tr>';
								
								echo '<tr><th><Label>Teaching Assistant:&nbsp;</Label></th><th><div class=form-group><select name=TA class=form-control>';
								$result = mysqli_query($con,"SELECT Name, StaffID FROM staff");						
								while($courses = mysqli_fetch_array($result))
								{
									$StaffName = $courses['StaffID'];
